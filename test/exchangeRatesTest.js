  ...
  describe("get", () => {
      ...
      try {
        ...
        mysqlStub.query = (query, params) => {
          ...
          if (query.startsWith(`INSERT INTO exchange_rates`)) {
            assert.ok(
              query.includes(
                `(from_currency, to_currency, rate, on_date) VALUES (?,?,?,?)`
              )
            );
      ...
      try {
        mysqlStub.query = (query, params) => {
          if (query.startsWith("SELECT rate, created_at FROM exchange_rates")) {
            assert.ok(
              query.includes(
                `WHERE from_currency = ? AND to_currency = ? AND on_date = ?`
              )
            );
            assert.deepEqual(
              params,
              ["AUD", "USD", today],
              "fromCurrency, then toCurrency then onDate respectively"
            );
            return [
              { rate: "0.7427190", created_at: "2018-07-23T06:58:45.000Z" }
            ];
          }
        };
    ...
    it("should respond with proper error message if API sends a bad request", async () => {
      try {
        ...
        assert.equal(
          err.message,
          "Symbols 'NPR' are invalid for date 2018-10-11."
        );
      }
    });
    ...
    it("should paginate the result with the limit of 10 rows per page", async () => {
      try {
        mysqlStub.query = (query, params) => {
          if (
            query.startsWith(`SELECT from_currency, to_currency`) &&
